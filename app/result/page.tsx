...
export default function ResultPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const res = await fetch(`/api/select?Q=${Q}&H=${H}&freq=${freq}`);
...
if (data && data.matched && data.matched.length > 0) {
  qMin = Math.min(...data.matched.map((t: any) => Number(t.q_min)));
  qMax = Math.max(...data.matched.map((t: any) => Number(t.q_max)));
  hMin = Math.min(...data.matched.map((t: any) => Number(t.h_min)));
  hMax = Math.max(...data.matched.map((t: any) => Number(t.h_max)));
}
